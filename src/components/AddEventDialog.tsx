...
export function AddEventDialog({ open, onOpenChange, onSubmit }: Props) {
  const [formData, setFormData] = useState({
    title: "",
    description: "",
    eventDate: "",
  ...
  const normalizeTime = (time: string) => {
    // Ensure proper time format for LocalTime parsing
    if (!time) return "00:00:00";
    
    // If time is in HH:MM format, add seconds
    if (/^\d{2}:\d{2}$/.test(time)) return `${time}:00`;
    
    // If time is in H:MM format, add leading zero and seconds
    if (/^\d{1}:\d{2}$/.test(time)) return `0${time}:00`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      date: formData.eventDate, // YYYY-MM-DD (LocalDate accepts this)
      time: normalizeTime(formData.eventTime), // HH:mm:ss (LocalTime accepts this)
      recipientEmails,
    };
